...
- **Env vars**: Create `.env` in project root with `SUPABASE_URL`, `SUPABASE_KEY`, and `SECRET_KEY` (no `.env.example` exists)
...
- `web/` - Flask app
  - `controllers/` - Blueprint route handlers
    ...
    - `pacientes.py`, `citas.py`, `inasistencias.py`, `censo.py`, `reportes.py`, `usuarios.py` - all under `/gestion` prefix
